  ...
  <div>
    ...
    <el-form ref="queryForm" :inline="true" :inline-message="true" size="small" style="background:#FFFFF">
      ...
      <el-form-item label="活动名称:">
        <el-select size="small" v-model="region.actId" placeholder="请选择活动" style="width: 200px">
    ...
    <table-wrapper :filterable="false" :excel="true" height="510px">
      <!-- 标题 -->
      <div slot="title">问卷调查活动答题次数统计</div>
      <!-- 列表 -->
      <el-table
        v-loading="loading"
        ...
        <el-table-column prop="actId" width="250" label="ID" show-overflow-tooltip></el-table-column>
        ...
        <el-table-column prop="provName" width="100" label="地区"></el-table-column>
        <el-table-column prop="title" width="300" label="问题"></el-table-column>
        <template v-for="item in columns">
          <el-table-column :key="item" :label="item" :prop="item+'##count'" align="center">
          </el-table-column>
        </template>
...
import * as TableUtil from './TableUtil.js'
  ...
  methods: {
    ...
    getActList () {
      ...
      trackApi.actList(option).then(res => {
        ...
        this.region.actId = this.actList[0].id
      })
    },
    handleChange () {},
    onChange (row, column, cell, event) {
      homeApi.getPrizeSend({ actId: row.actId }).then(res => {
    ...
    doSearch () {
      ...
      // this.region.actId = 'f3dec1f9bb7011ea8019506b4b2303f4'
      trackApi.paperStatistics(this.region).then(res => {
        this.dataList = res.value
        this.lists = TableUtil.handleArrayTransform(this.dataList, ['actId', 'name', 'provName', 'title'], ['answer', 'count'], ['answer'])
        this.columns = [...new Set(this.dataList.map(item => { return item.answer }))]
        this.loading = false
      })
    },
    // 赋值活动链接
