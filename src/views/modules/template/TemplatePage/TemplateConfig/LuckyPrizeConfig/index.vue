<template>
  <div class="lucky-prize-form">
    <table-wrapper :style="{height:'100%'}" :filterable="false" :excel="false">
      <!-- 标题 -->
      <div slot="title">
        {{luckyName}}
      </div>
    ...
    <prize-img ref="prize-img" :prizeRow="prizeRow" @refreshIframe="refreshIframe"></prize-img>
  </div>
</template>
...
import PrizeChance from './PrizeChance'
import PrizePoolTemplate from './PrizePoolTemplate'
  ...
  methods: {
    ...
    handleSearch () {
      ActLuckyApi.detail(this.phaseId).then(res => {
        this.luckyId = res.data.id
        this.luckyName = res.data.luckyName
        this.luckyBeginTime = res.data.beginTime
        this.luckyEndTime = res.data.endTime
    ...
    handleMaterialMapping (row) {
      this.prizeRow = row
      this.$refs['prize-img'].dialogFormVisible = true
      // const { href } = this.$router.resolve({
      //   name: 'ActMaterialMapping',
      //   query: { sourceId: row.id, sourceType: 'PRIZE', sourceName: row.name }
      // })
