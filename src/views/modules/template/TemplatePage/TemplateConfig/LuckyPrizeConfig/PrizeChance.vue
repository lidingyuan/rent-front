    ...
    <table-wrapper :filterable="false" :excel="false" :expand="false" height="70vh">
      ...
      <el-table :data="dataList" size="small" height="100%" border stripe @selection-change="handleSelectionChange">
        ...
        <template v-for="item in validPrizeList">
          <el-table-column :key="item.id" :label="item.name" :prop="item.id+'##chance'" width="80" align="right" show-overflow-tooltip></el-table-column>
        </template>
          ...
          <el-col :span="12" v-for="item in tempList" :key="item.prizeId">
            <el-form-item :label="item.prizeName" label-width="160px">
              <el-tooltip effect="dark" content="设置数量" placement="top" style="width:70%;">
                <el-input-number v-model="item.chance" placeholder="" :min="0" :precision="4" :step=".1" ></el-input-number>
              </el-tooltip>
            </el-form-item>
          </el-col>
          <el-col :span="24">
            <el-form-item label="" label-width="160px">
              <span style="color: #F56C6C">中奖率合计：{{totalChance}} <span v-if="totalChance === 1">&nbsp;&nbsp;Perfect ！！！</span></span>
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
...
import * as ActLuckyPrizeChanceApi from '@/api/lucky/ActLuckyPrizeChanceApi'
...
export default {
  ...
  computed: {
    ...
    validPrizeList () {
      return this.prizeList.filter(item => item.enabled === 1)
    },
    totalChance () {
      let total = this.tempList.reduce((accumulator, currentValue) => accumulator + (currentValue.chance || 0), 0)
  ...
  methods: {
    ...
    handleSearch () {
      ...
      ActLuckyPrizeChanceApi.listForTransform(options).then(res => {
    ...
    createData () {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          ActLuckyPrizeChanceApi.saveOrUpdateBatch(saveDataList).then(res => {
            this.handleSearch()
    ...
    updateData () {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          ActLuckyPrizeChanceApi.saveOrUpdateBatch(saveDataList).then(() => {
            this.handleSearch()
            this.dialogFormVisible = false
    ...
    handleDelete (row) {
      this.$confirm('删除中奖率会导致此周期内无法出奖。提交后1分钟内生效，是否确认删除?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        let ids = this.validPrizeList.map(prize => row[prize.id + '##id'])
        ActLuckyPrizeChanceApi.del(ids).then(res => {
    ...
    resetTemp (row) {
      ...
      this.tempList = this.validPrizeList.map(prize => {
        return {
          ...
          chance: row ? row[prize.id + '##chance'] : 0
        }
      })
    },
    // ---
