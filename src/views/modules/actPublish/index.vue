...
export default {
  ...
  methods: {
    ...
    handleUpdate (row) {
      if (row.status === 1) {
        this.$message.success('该活动已发布')
        return
      }
      ActConfigApi.updateStatus({ actId: row.id, status: 1 }).then(() => {
    ...
    // --- 活动模板修改
    handleTemplate (row) {
      let url = location.href.substr(0, location.href.indexOf('#')) + '#/actTemplate/apply?actId=' + row.id
      window.open(url)
    }
  }
}
