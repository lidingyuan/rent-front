...
const secondsToDayFilter = (value) => {
  const sub = !value ? 0 : Math.ceil((new Date().getTime() - value * 1000) / (1000 * 3600 * 24))
  return !sub ? '暂无' : sub + '天'
}
...
const prov2Name = (code) => {
  ...
  const prov = provList.find(prov => prov.provCode === code)
...
const city2Name = (code, level = 2) => {
  ...
  const city = cityList.find(city => city.cityCode === code)
